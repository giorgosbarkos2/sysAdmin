...
function renderUI(obj) {		
	obj.html(
		'<div class="plupload_wrapper" style="position:relative:width:500px;height:200px;margin-top-200px;">' +
				...
				'<div class="plupload">' +
					...
					'<div class="plupload_content">' +
						...
						'<tr class="ui-widget-header plupload_filelist_header">' +
							'<td class="plupload_cell plupload_file_name">' + ('Nombre Archivos') + '</td>' +
							'<td class="plupload_cell plupload_file_status">' + ('Estatus') + '</td>' +
							'<td class="plupload_cell plupload_file_size">' + ('Peso') + '</td>' +
...
$.widget("ui.plupload", {
	...
	_create: function() {
		...
		// buttons
		this.browse_button = $('.agregar', this.container).attr('id', id + '_browse');
	...
	_updateFileList: function() {
		...
		if (up.total.queued === 0) {
			$('.ui-button-text', self.browse_button).html(('A '));
		} else {
			$('.ui-button-text', self.browse_button).html(('%d Archi ').replace('%d', up.total.queued));
		}
		...
		if (!up.files.length && up.features.dragdrop && up.settings.dragdrop) {
			...
			$('#' + id + '_filelist').append('<tr><td class="plupload_droptext">' + _("Arrastra Archivos aqui.") + '</td></tr>');
		} else {
	...
	_enableDragAndDrop: function() {
		this.filelist.append('<tr><td class="plupload_droptext">' + _("Arrastra archivos aca.") + '</td></tr>');
	...
	destroy: function() {
		...
		$('.boton', this.element).unbind();
		
		// destroy buttons
		if ($.ui.button) {
			$('.agregar, .plupload_start, .plupload_stop', this.container)
				.button('destroy');
		}
